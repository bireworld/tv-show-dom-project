...
function displayOne(width, height, length) {
  if (length === 1) {
    document
      .getElementsByClassName("episode-img")[0]
      .setAttribute("style", `width:${width}rem;height:${height}rem`);
    document
      .getElementsByClassName("episode-description")[0]
      .setAttribute("style", `width:${width}rem;height:auto`);
  }
  if (length === 2) {
    document
      .getElementsByClassName("episode-img")[0]
...
function search(txtSearch) {
  ...
  for (let i = 0; i < List.length; i++) {
    if (
      ...
      if (
        List[i].name.toLowerCase().includes(txtSearch.value.toLowerCase()) ||
        List[i].summary.toLowerCase().includes(txtSearch.value.toLowerCase())
      ) {
        searchList.push({
          name: List[i].name,
          img: List[i].image.medium,
  ...
  if (searchList.length > 0) {
    for (let j = 0; j < searchList.length; j++) {
      if (
        searchList[j].name != null &&
        searchList[j].summary != null &&
        searchList[j].img != null &&
        searchList[j].number != null &&
        searchList[j].season != null
      ) {
        display(
          searchList[j].name,
          searchList[j].img,
          searchList[j].summary,
          searchList[j].season,
          searchList[j].number
        );
      }
  ...
  match.textContent = "";
  match.textContent =
    "Displaying " + searchList.length + "/" + List.length + " Episodes";
}
...
txtSearch.addEventListener("keyup", function () {
  ...
  if (txtSearch.value === "") {
    ...
    match.textContent =
      "Displaying " + List.length + "/" + List.length + " Episodes";
  } else {
    btnEpisodeBack.remove();
    search(txtSearch);
  }
...
function episodeHeadingClick(episodeLink) {
  ...
  for (let obj of List) {
    if (obj.name === epName) {
      ...
      btnEpisodeBack.setAttribute(
        "style",
        "width:5rem;height:2rem;font-weight:bold;margin-left:0.5rem;"
      );
...
function makePageForEpisodes(episodeList) {
  ...
  for (i = 0; i < episodeList.length; i++) {
    if (
      episodeList[i].name != null &&
      episodeList[i].summary != null &&
      episodeList[i].image != null &&
      episodeList[i].number != null &&
      episodeList[i].season != null
    ) {
      display(
        episodeList[i].name,
