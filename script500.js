...
function showList(show) {
  ...
  for (let obj of show) {
    ...
    if (showCount === 100) {
      ...
      btnLoadMore.setAttribute(
        "style",
        "float:right;width:10rem;font-weight:bold"
      );
      break;
    }
  }
  btnLoadMore.addEventListener("click", function () {
    ...
    btnShowLess.setAttribute(
      "style",
      "width:10rem;font-weight:bold;float:right;"
    );
  ...
  if (txtFiltering.value != "") {
    termsNAmeSummary = show.filter(function (obj) {
      return (
        obj.name
          .toLocaleLowerCase()
          .includes(txtFiltering.value.toLocaleLowerCase()) ||
        obj.summary
          .toLocaleLowerCase()
          .includes(txtFiltering.value.toLocaleLowerCase())
      );
